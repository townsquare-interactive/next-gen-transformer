...
export async function scrapeAssetsFromSite(settings) {
    try {
        ...
        console.error('Error during scrapeAssetsFromSite:', err)
    ...
    page.on('response', async (response: Response) => {
        ...
        if (response.request().resourceType() === 'image') {
            ...
            response.body().then(async (fileContents) => {
                const imageFileName = hashUrl(response.url())
...
async function scrape(settings) {
    ...
    page.on('response', async (response) => {
        ...
        if (response.request().resourceType() === 'image') {
            ...
            response.body().then(async (fileContents) => {
                const imageFileName = hashUrl(response.url()) // Hash the image URL to create a unique name
                const fileExtension = path.extname(url.pathname) || '.jpg' // Default to .jpg if no extension
                const fileName = `${imageFileName}${fileExtension}`
                const filePath = path.resolve(settings.storagePath, fileName)
                console.debug(`url = ${url}, filePath = ${filePath}`)
