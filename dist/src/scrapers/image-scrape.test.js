...
import { scrapeAssetsFromSite } from './image-scrape.js'
...
describe('Scrrape Images For Duda', () => {
    ...
    it('should return an array with the scraped image strings with a valid url', async () => {
        const result = await scrapeAssetsFromSite({ url: url, storagePath: 's3' })
    ...
    it('should fail when the URL is not a valid site', async () => {
        ...
        let error
        try {
            await scrapeAssetsFromSite({ url: errUrl, storagePath: 's3' })
        } catch (err) {
