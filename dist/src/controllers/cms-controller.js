...
const transformPagesData = (pageData, sitePageData, themeStyles, basePath) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    for (const [key, value] of Object.entries(pageData)) {
        ...
        if (value.data.modules) {
            ...
            value.data = Object.assign(Object.assign({ id: pageId, title: pageTitle, slug: pageSlug, pageType: pageType, url: url }, value.data), { columnStyles: columnStyles });
            ...
            const currentFile = yield getFileS3(`${basePath}/pages/${pageSlug}.json`);
            const newSeoFile = Object.assign(Object.assign({}, currentFile), { seo: value.seo });
...
const createPageScss = (pageData, pageSlug, basePath) => __awaiter(void 0, void 0, void 0, function* () {
    let pageCss;
    ...
    yield addFileS3(pageCss, `${basePath}/styles/${pageSlug}`, 'scss');
});
const deletePages = (pages, basePath) => __awaiter(void 0, void 0, void 0, function* () {
    console.log('deleter started');
    const oldPageList = yield getFileS3(`${basePath}/pages/page-list.json`);
    let newPageList = [];
    for (let i = 0; i < oldPageList.pages.length; i++) {
        if (!(oldPageList.pages[i].id in pages)) {
            ...
            yield deleteFileS3(`${basePath}/pages/${oldPageList.pages[i].slug}.json`);
        }
    }
    return newPageList ? { pages: newPageList } : oldPageList;
});
//Update pagelist file in s3 or create if not already there
const updatePageList = (page, basePath) => __awaiter(void 0, void 0, void 0, function* () {
    console.log('page list updater started ------');
    const pageListUrl = `${basePath}/pages/page-list.json`;
    let pageListFile = yield getFileS3(`${basePath}/pages/page-list.json`);
    addPagesToList(pageListFile, page, basePath);
    //Can use add file when ready, instead of addpagelist logging
    yield addFileS3List(pageListFile, pageListUrl);
    return pageListFile;
});
//add page object to pagelist
const addPagesToList = (pageListFile, page, basePath) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    for (let i = 0; i < page.length; i++) {
        ...
        if (pageListFile.pages.filter((e) => e.name === pageData.title).length === 0) {
            ...
            yield addNewPageToNav(pageData, basePath);
        }
    }
});
const addNewPageToNav = (pageData, basePath) => __awaiter(void 0, void 0, void 0, function* () {
    //Get layout file to update nav
    let oldSiteData = yield getFileS3(`${basePath}/layout.json`);
    if (oldSiteData) {
        //add new page to nav
        ...
        yield addFileS3(oldSiteData, `${basePath}/layout`);
    }
});
//Create or edit layout file
const createOrEditLayout = (file, basePath, themeStyles) => __awaiter(void 0, void 0, void 0, function* () {
    console.log('layout edit');
    const currentLayout = yield getFileS3(`${basePath}/layout.json`);
        ...
        for (let i = 0; i < file.settings.social.services.length; i++) {
            ...
            if (file.settings.social.services[i]) {
                if (item.value && item.enabled == 1) {
                    social.push(Object.assign(Object.assign({}, item), { url: basePath, icon: socialConvert(item.name) }));
                }
            }
        }
        return social;
        ...
        if (moduleList[i]) {
            ...
            for (const [key, value] of Object.entries(moduleList[i])) {
                ...
                for (let i = 0; i < value.items.length; i++) {
                    ...
                    value.items[i] = Object.assign(Object.assign({}, value.items[i]), { buttonList: buttonList, imageIcon: imageIcon, linkNoBtn: linkNoBtn, twoButtons: twoButtons, isWrapLink: isWrapLink, visibleButton: visibleButton, isBeaconHero: isBeaconHero, imagePriority: imagePriority, 
                        //hasGridCaption: hasGridCaption,
                        itemCount: itemCount, btnStyles: btnStyles, nextImageSizes: nextImageSizes, isFeatureButton: isFeatureButton });
                        ...
                        const imageType = !['no_sizing', 'no_set_height'].includes(value.imgsize)
                            ...
                            : modRenderType === 'Banner'
                                ...
                                : modRenderType === 'Parallax'
                                    ? 'crop'
                                    : 'nocrop';
                        value.items[i] = Object.assign(Object.assign({}, value.items[i]), { imageType: imageType });
                    }
                }
                ...
                if (modRenderType === 'ContactFormRoutes') {
                    ...
                    newModule = Object.assign(Object.assign({}, newModule), { contactFormData: contactFormData });
                }
                const modData = Object.assign(Object.assign({}, newModule), { modId: key, modCount: modCount, columnLocation: i, isSingleColumn: isSingleColumn });
...
const createGlobalStylesheet = (themeStyles, fonts, code, currentPageList, basePath) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    ${code.CSS}
    `;
    const allPageStyles = yield getAllCssPages(currentPageList, basePath);
...
const getAllCssPages = (currentPageList, basePath) => __awaiter(void 0, void 0, void 0, function* () {
    const allPageCss = [];
    for (let i = 0; i < currentPageList.pages.length; i++) {
        const pageSlug = currentPageList.pages[i].slug;
        const cssFile = yield getCssFile(pageSlug, basePath);
